...
export default function Header() {
  ...
  // Detecta a localização do visitante via API de geolocalização por IP
  useEffect(() => {
    fetch('https://ipapi.co/json/')
      .then((res) => res.json())
      .then((data) => {
        if (data && data.city) {
          setVisitorLocation({ city: data.city, country: data.country_name });
        }
      })
      .catch((err) => {
        console.error('Não foi possível detectar a localização do visitante', err);
      });
  }, []);

  const handleWhatsAppClick = () => {
    const phoneNumber = '5513997546382';

    const message = visitorLocation && visitorLocation.city
      ? 'Olá! Vi o site do Sindify Gestão (acessando de ' + visitorLocation.city + ') e gostaria de mais informações sobre o sistema.'
      : 'Olá! Vi o site do Sindify Gestão e gostaria de mais informações sobre o sistema.';
